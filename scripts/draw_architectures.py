...
def draw_unet(ax):
    ...
    bw, bh = 1.6, 0.55  # block width, height

    # Input
    draw_block(ax, 1.0, 6.5, 1.4, 0.45, 'Input\n256×256×3', COLORS['input'], fontsize=6)

    # Encoder (left side - going down)
    enc_labels = ['ResNet Stage1\n128×128×64', 'ResNet Stage2\n64×64×128',
                  'ResNet Stage3\n32×32×256', 'ResNet Stage4\n16×16×512']
    enc_y = [5.5, 4.3, 3.1, 1.9]
    enc_positions = []
    for i, (label, y) in enumerate(zip(enc_labels, enc_y)):
        pos = draw_block(ax, 1.0, y, bw, bh, label, COLORS['encoder'], fontsize=5.5)
        enc_positions.append(pos)

    # Arrows encoder
    draw_arrow(ax, 1.0, 6.5-0.22, 1.0, 5.5+0.28, COLORS['encoder'])
    for i in range(len(enc_y)-1):
        draw_arrow(ax, 1.0, enc_y[i]-0.28, 1.0, enc_y[i+1]+0.28, COLORS['encoder'])

    # Bottleneck
    draw_block(ax, 5.0, 0.5, 2.0, 0.55, 'Bottleneck 8×8×512', COLORS['bottleneck'], fontsize=6)
    draw_arrow(ax, 1.0, 1.9-0.28, 1.0, 0.5, COLORS['encoder'])
    draw_arrow(ax, 1.8, 0.5, 4.0, 0.5, COLORS['bottleneck'])
    ...
    dec_positions = []
    for i, (label, y) in enumerate(zip(dec_labels, dec_y)):
        pos = draw_block(ax, 9.0, y, bw, bh, label, COLORS['decoder'], fontsize=5.5)
        dec_positions.append(pos)

    # Arrows decoder
    draw_arrow(ax, 6.0, 0.5, 9.0, 1.9-0.28, COLORS['decoder'])
    for i in range(len(dec_y)-1):
        draw_arrow(ax, 9.0, dec_y[i]+0.28, 9.0, dec_y[i+1]-0.28, COLORS['decoder'])
    ...
    # Output
    draw_block(ax, 9.0, 6.5, 1.4, 0.45, 'Output\n256×256×C', COLORS['output'], fontsize=6)
    draw_arrow(ax, 9.0, 5.5+0.28, 9.0, 6.5-0.22, COLORS['decoder'])
...
def draw_segnet(ax):
    ...
    enc_labels = ['VGG Block1\n128×128×64', 'VGG Block2\n64×64×128',
                  'VGG Block3\n32×32×256', 'VGG Block4\n16×16×512',
                  'VGG Block5\n8×8×512']
    enc_y = [5.5, 4.5, 3.5, 2.5, 1.5]
    for i, (label, y) in enumerate(zip(enc_labels, enc_y)):
        draw_block(ax, 1.0, y, bw, bh, label, COLORS['encoder'], fontsize=5.5)

    draw_arrow(ax, 1.0, 6.5-0.22, 1.0, 5.5+0.28, COLORS['encoder'])
    for i in range(len(enc_y)-1):
        draw_arrow(ax, 1.0, enc_y[i]-0.28, 1.0, enc_y[i+1]+0.28, COLORS['encoder'])
        # MaxPool + store indices
    ...
    for i, (label, y) in enumerate(zip(dec_labels, dec_y)):
        c = COLORS['output'] if i == 4 else COLORS['decoder']
        draw_block(ax, 9.0, y, bw, bh, label, c, fontsize=5.5)
    ...
    # Pooling indices transfer (curved arrows)
    for i in range(5):
        enc_x = 1.0 + bw/2
        dec_x = 9.0 - bw/2
        y_enc = enc_y[i]
    ...
    # Bottom connection
    draw_arrow(ax, 1.8, 1.5, 8.2, 1.5, COLORS['bottleneck'], lw=1.5)
...
def draw_deeplabv3plus(ax):
    ...
    # Encoder
    enc_labels = ['ResNet Stage1\n64×64×64', 'ResNet Stage2\n32×32×128',
                  'ResNet Stage3\n16×16×256', 'ResNet Stage4\n8×8×512']
    enc_y = [5.3, 4.1, 2.9, 1.7]
    for i, (label, y) in enumerate(zip(enc_labels, enc_y)):
        draw_block(ax, 0.8, y, bw, bh, label, COLORS['encoder'], fontsize=5.5)

    draw_arrow(ax, 0.8, 6.5-0.22, 0.8, 5.3+0.28, COLORS['encoder'])
    for i in range(len(enc_y)-1):
        draw_arrow(ax, 0.8, enc_y[i]-0.28, 0.8, enc_y[i+1]+0.28, COLORS['encoder'])

    # ASPP Module (center)
    aspp_x, aspp_y = 5.0, 1.7
    draw_block(ax, aspp_x, aspp_y, 2.8, 0.7, 'ASPP Module', COLORS['aspp'],
               fontsize=7, text_color='white')

    # ASPP branches
    branches = ['1×1\nconv', 'rate\n6', 'rate\n12', 'rate\n18', 'Global\nAvgPool']
    branch_x = [3.8, 4.4, 5.0, 5.6, 6.2]
    for i, (label, bx) in enumerate(zip(branches, branch_x)):
        draw_block(ax, bx, 0.5, 0.5, 0.5, label, '#E8C547', fontsize=4, text_color='#333')

    # Arrows from encoder to ASPP
    draw_arrow(ax, 0.8 + bw/2, 1.7, aspp_x - 1.4, 1.7, COLORS['encoder'], lw=1.5)

    # Arrows from ASPP to branches and back
    for bx in branch_x:
        draw_arrow(ax, bx, aspp_y - 0.35, bx, 0.75, '#E8C547', lw=0.7)

    # Concat from ASPP branches
    draw_block(ax, 5.0, -0.3, 1.8, 0.4, 'Concat + 1×1 Conv', COLORS['bottleneck'], fontsize=5.5)
    for bx in branch_x:
        draw_arrow(ax, bx, 0.25, 5.0, -0.1, '#999', lw=0.5)

    # Decoder
    draw_block(ax, 8.5, 2.0, 1.8, 0.6, 'Upsample 4×\n+ 1×1 Conv', COLORS['decoder'], fontsize=5.5)
    draw_block(ax, 8.5, 3.5, 1.8, 0.6, 'Concat\n+ 3×3 Conv', COLORS['decoder'], fontsize=6)
    draw_block(ax, 8.5, 5.0, 1.8, 0.55, 'Upsample 4×\n256×256×C', COLORS['decoder'], fontsize=5.5)

    # Arrow: ASPP concat → decoder upsample
    draw_arrow(ax, 5.9, -0.3, 8.5, 1.7, COLORS['decoder'], lw=1.2)

    # Arrow: decoder blocks
    draw_arrow(ax, 8.5, 2.3, 8.5, 3.2, COLORS['decoder'])
    draw_arrow(ax, 8.5, 3.8, 8.5, 4.72, COLORS['decoder'])

    # Low-level feature skip (from Stage1 to decoder concat)
    ax.annotate('', xy=(7.6, 3.5), xytext=(0.8 + bw/2, 5.3),
                arrowprops=dict(arrowstyle='->', color=COLORS['skip'],
                               lw=1.8, linestyle='--',
                               connectionstyle='arc3,rad=0.3'))
    ax.text(4.5, 4.8, 'Low-level\nfeatures', ha='center', va='center',
            fontsize=5.5, color=COLORS['skip'], fontstyle='italic',
            bbox=dict(boxstyle='round,pad=0.2', facecolor='white',
                     edgecolor=COLORS['skip'], alpha=0.7))

    # Output
    draw_block(ax, 8.5, 6.2, 1.4, 0.45, 'Output\n256×256×C', COLORS['output'], fontsize=6)
    draw_arrow(ax, 8.5, 5.28, 8.5, 6.2-0.22, COLORS['decoder'])
...
combined_path = 'outputs_topic05_3/architecture_diagrams_combined.png'
...
for name, draw_func in [('unet', draw_unet),
    ...
    path = f'outputs_topic05_3/architecture_{name}.png'
